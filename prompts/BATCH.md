...
| 2 | `02_feedback_loop.md` | Reconcile all design documents against the implemented codebase. All later steps depend on the reconciled docs this produces. |
| 3 | `03_rewrite_glossary.md` | Rewrite the glossary against the actual implementation. Depends on reconciled docs from step 2. |
| 4 | `04_generate_walkthrough.md` | Generate the `walkthrough/` directory. Depends on reconciled docs from step 2. |
| 5 | `05_generate_diagrams.md` | Generate Mermaid diagrams for README and walkthrough. Depends on reconciled docs (step 2) and walkthrough directory (step 4). |
| 6 | `06_readme_generation.md` | Generate README and all documentation artifacts. Depends on reconciled docs (step 2), walkthrough (step 4), and diagrams (step 5). |
